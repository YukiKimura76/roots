    ...
    <body class="font-sans antialiased">
        <div class="relative min-h-screen md:flex" x-data="{ open: true }">
            ...
            <aside :class="{ '-translate-x-full': !open }" class="z-10 bg-blue-800 text-blue-100 w-64 px-2 py-4 absolute inset-y-0 left-0 md:relative transform  md:translate-x-0
                ...
                <!-- Nav links -->
                <nav>
                    <x-side-nav-link href="{{ route('dashboard') }}" :active="route()->routeIs('dashboard')">
                        Dashboard
                    </x-side-nav-link>
                    <x-side-nav-link href="{{ route('features') }}" :active="route()->routeIs('dashboard')">>
                        Features
                    </x-side-nav-link>
                    <x-side-nav-link href="{{ route('about') }}" :active="route()->routeIs('dashboard')">>
                        About
                    </x-side-nav-link>
                    <x-side-nav-link href="{{ route('contact') }}" :active="route()->routeIs('dashboard')">>
                        Contact
                    </x-side-nav-link>
