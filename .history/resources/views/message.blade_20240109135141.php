          ...
          <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
              <div class="p-6 text-gray-900 dark:text-gray-100">
                ...
                <section class="text-gray-600 body-font">
                  ...
                  <div class="text-white bg-gray-500 border-0 py-2 px-8 focus:outline-none hover:bg-gray-600 rounded text-lg font-bold">登録済み設定一覧</button>
                  </div>
                    <!--右側エリア[START]-->
                    ...
                    @if (count($msgs) > 0)
                        @foreach ($msgs as $message)
                            <x-collection id="{{ $message->id }}">
                              {{ $message->sender_name }}（{{ $message->date }}年まで）
                            </x-collection>
                        @endforeach
                    @endif
